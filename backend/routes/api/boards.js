...
// @access Authorized Users
/*********************************************/
router.get('/:boardid', authBoard, (request, response) => {
    // Check for board Access
    logger.log('GET api/boards/:boardid', 'request.user.name: ' +request.user.name );
    logger.log('GET api/boards/:boardid', 'Passed board id is: ' +request.params.boardid );
    if(request.boardAccess){
        logger.log('GET api/boards/:boardid', 'request.user.boardAccess: ' +request.boardAccess );
        if(request.boardAccess ==='Admin' || request.boardAccess ==='User') {
            // Get task lists for board
            taskList.find({boardId: request.params.boardid})
                .then(taskList => { 
                    response.json({boardId: request.params.boardid, taskList});
                })
...
router.delete('/:boardid', authBoard, (request, response) => {
    // Check for board Access
    logger.log('DELETE api/boards/:boardid', 'request.user.name: ' +request.user.name );
    logger.log('DELETE api/boards/:boardid', 'Passed board id is: ' +request.params.boardid );
    if(request.boardAccess){
        logger.log('DELETE api/boards/:boardid', 'request.user.boardAccess: ' +request.boardAccess );
